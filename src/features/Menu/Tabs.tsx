...
export const TabComponent = ({ modal, onChange = () => {} }: TabComponentProps) => {
    const navigate = useNavigate({ from: "/menu/view/$id" });
    const search = useSearch({ from: "/_main/menu/view/$id" });
    const [selectedLang, setSelectedLang] = useState<string>(search.lang ? search.lang : "oz");
    ...
    useEffect(() => {
        if (search.lang && search.lang !== selectedLang) {
            setSelectedLang(search.lang ? search.lang : "oz");
        }
